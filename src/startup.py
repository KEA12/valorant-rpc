...
class Startup:
    def __init__(self):
        if not Processes.is_program_already_running():
            ...
            if Mappings.get_config_value("region", 0) == "":
                self.check_region()
    ...
    def setup_client(self):
        try:
            self.client = valclient.Client(region = Mappings.get_config_value("region", 0))
            ...
            if session["productId"] == "valorant":
                ...
                for arg in launch_args:
                    if "-ares-deployment" in arg:
                        ...
                        self.config[Mappings.get_config_key("region")][0] = region
